...
class TECStimulus(Stimulus):
    ...
    def __init__(self, port="COM3", channel=1, queries=DEFAULT_QUERIES, *args, **kwars):
        ...
        with MeCom("COM3") as mc:
            ...
            print(
                "connected to device: {}, status: {}".format(self.address, self.status)
            )
...
class SetTECtemperatureStimulus(TECStimulus):
    ...
    def start(self):
        ...
        with MeCom("COM3") as mc:
            ...
            temp = mc.get_parameter(
                parameter_name="Object Temperature", address=self.address
            )
            print("query for object temperature, measured temperature {}C".format(temp))


class InterpolatedTECTemperatureStimulus(
    InterpolatedStimulus, SetTECtemperatureStimulus
):
    ...
    def update(self):
        ...
        if self.k == 0:
            with MeCom("COM3") as mc:
                ...
                self.object_temperature = mc.get_parameter(
                    parameter_name="Object Temperature", address=self.address
                )
                # print("query for object temperature, measured temperature {}C".format(temp))
                self.output_current = mc.get_parameter(
                    parameter_name="Actual Output Current", address=self.address
                )
                # print("query for output current, measured current {}A".format(out_current))
                self.output_voltage = mc.get_parameter(
                    parameter_name="Actual Output Voltage", address=self.address
                )
                # print("query for output voltage, measured voltage {}V".format(out_voltage))
                self.device_status = mc.get_parameter(
                    parameter_name="Device Status", address=self.address
                )
...
if __name__ == "__main__":
    ...
    # stim.start()
    stim = InterpolatedTECTemperatureStimulus(
        df_param=pd.DataFrame(
            dict(t=[0, 5, 5, 10], temperature=[25.0, 25.0, 23.0, 23.0])
        )
    )
    stim.update()
