...
######################################################
# lib = cdll.LoadLibrary(r"C:\Program Files\Thorlabs\Kinesis\Thorlabs.MotionControl.Benchtop.BrushlessMotor.dll")
...
class MOT_StatusBits(Structure):
    _fields_ = [("1", c_word),
                ("2", c_word),
                ("3", c_word),
                ("4", c_dword),
...
BBMC_RequestPosLoopParams = bind(lib, "BMC_RequestPosLoopParams",[POINTER(c_char), c_short], c_short)
...
#Todo write these functions and test for output
############################
...
BMC_GetTrackSettleParams = bind(lib, "BMC_GetTrackSettleParams", [POINTER(c_char), c_short, POINTER(MOT_BrushlessTrackSettleParameters)], c_short)
BMC_SetTrackSettleParams = bind(lib, "BMC_SetTrackSettleParams", [POINTER(c_char), c_short, POINTER(MOT_BrushlessTrackSettleParameters)],c_short)
BMC_RequestSettledCurrentLoopParams = bind(lib, "BMC_RequestSettledCurrentLoopParams", [POINTER(c_char), c_short])
BMC_RequestTrackSettleParams = bind(lib, "BMC_RequestTrackSettleParams", [POINTER(c_char), c_short])
...
BMC_GetRackStatusBits = bind(lib, "BMC_GetRackStatusBits", [c_char_p, c_short], c_short)

# BMC_GetStatusBits = bind(lib, "BMC_GetStatusBits", [c_char_p, POINTER(MOT_StatusBits)], c_dword)
...
class Motor:
    ...
    def request_pid(self):
        err = BBMC_RequestPosLoopParams(self.serial_nom, self.channel)
        print ("request pid with error:", err)
    ...
    def set_pos_loop_params(
        ...
        posloop_info.factorForOutput =factor
        posloop_info.velocityFeedForward =velo
        posloop_info.accelerationFeedForward= acc
    ...
    def move_rel(self, distance):
        ...
        print ("moving rel error", err)


    def movejog(self, direction):
        err =BMC_MoveJog(self.serial_nom, self.channel, direction)
        print ("joging error",err)
    ...
    def set_jogstepsize(self, stepsize):
        err =BMC_SetJogStepSize(self.serial_nom, self.channel, stepsize)
        # print ("stepsize",err)
    ...
    def get_jog_params(self):
        ...
        err= BMC_GetJogParamsBlock(self.serial_nom, self.channel, byref(params))
    ...
    def get_settle_params(self):
        params = MOT_BrushlessTrackSettleParameters()
        err= BMC_GetTrackSettleParams(self.serial_nom, self.channel, byref(params))

        if err == 0:
            print("time: ", params.time)
            print("settledError: ", params.settledError)
            print("maxTrackingError: ", params.maxTrackingError)
            print("notUsed: ", params.notUsed)
            print("lastNotUsed: ", params.lastNotUsed)
        else:
            print ("error: ",err)

    def set_settle_params(self, time=int(), settledError=int(), maxTrackingError=int() , notUsed=int(), lastNotUsed=int()
    ):
        settleloop_info = MOT_BrushlessTrackSettleParameters()  # container
        settleloop_info.time = time
        settleloop_info.settledError = settledError
        settleloop_info.maxTrackingError = maxTrackingError
        settleloop_info.notUsed = notUsed
        settleloop_info.lastNotUsed = lastNotUsed

        BMC_SetTrackSettleParams(self.serial_nom, self.channel, byref(settleloop_info))
        print("New settle parameters set.")
    ...
    def request_status_bits(self):
        err = BMC_RequestStatusBits(self.serial_nom, self.channel)
        print ("requested status bits with error {}".format(err))

    def start_polling(self):
        err =BMC_StartPolling(self.serial_nom, self.channel, 250)
        print ("Called polling {}".format(err))

    def get_status_bits(self):
        output = BMC_GetStatusBits(self.serial_nom, self.channel)
        return output

    def status(self):
        print("getting status")
        # o = BMC_StartPolling(self.serial_nom, self.channel, 250)
        #         # print("Polling:", o)
        #         # sleep(3)
        container = MOT_StatusBits()
        if BMC_RequestStatusBits(self.serial_nom, self.channel) == 0:
            out = BMC_GetStatusBits(self.serial_nom, self.channel)
            # out = BMC_GetRackStatusBits(self.serial_nom, self.channel)
            print(type(out))
            print (out)
