...
class SpinnakerCamera(Camera):
    ...
    def open_camera(self):
        ...
        if not PySpin.IsAvailable(acquisition_mode_node) or not PySpin.IsWritable(acquisition_mode_node):
            messages.append("W: May not be able to set acquisition mode to continuous (enum retrieval).")
        ...
        if not PySpin.IsAvailable(acquisition_mode_continuous_node) or not PySpin.IsReadable(acquisition_mode_continuous_node):
            messages.append('W: Unable to set acquisition mode to continuous (entry retrieval).')
        ...
        try:
            ...
            for i, (s, o) in enumerate(zip(["Width", "Height"],
                                           ["OffsetX", "OffsetY"])):
                ...
                if np.mod(value_to_set, inc) != 0:
                    ...
                    messages.append("W: Need to set width in increments of {0}, resetting to {1}.".format(
                            inc, value_to_set))
                roi_node.SetValue(value_to_set)

                # offset
                offset_node = PySpin.CIntegerPtr(
                    nodemap.GetNode(o))
                if self.roi[0 + i] == -1:
            ...
            messages.append("E:Could not set ROI. Exeption: {0}.".format(ex))
        ...
        if PySpin.IsAvailable(node_acquisition_frame_rate_control_enable): #older simpler api
            ...
            if not PySpin.IsAvailable(enable_rate_mode) or not PySpin.IsWritable(enable_rate_mode):
                messages.append("W:enable_rate_mode not available/writable. Aborting...")
            try:
        ...
        try:
            if param == "exposure":  #sent in ms
                ...
                if exposure_time_to_set > self.exposure_max:
                    ...
                    messages.append("E:exposure less than min of {:3.1f}".format(
            ...
            if param == "gain":
                ...
                if gain_to_set > self.gain_max:
                    messages.append("E:gain greater than max of {:3.1f}".format(self.gain_max))
                    gain_to_set = self.gain_max
                elif gain_to_set < self.gain_min:
                    messages.append("E:gain less than min of {:3.1f}".format(self.gain_min))
            ...
            if param == "framerate":
                ...
                if frame_rate > self.rate_max:
                    messages.append("E:fps greater than max of {:3.1f}".format(self.rate_max))
                    frame_rate = self.rate_max
                elif frame_rate < self.rate_min:
                    messages.append("E:fps less than min of {:3.1f}".format(self.rate_min))
